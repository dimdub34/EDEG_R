...
Aanalyse the relation between work importance (trav.imp) and gender.
...
Copy/paste the code below and analyse the relation between the variable `age` and the variable `smoker`

```{r}
set.seed(123)  # for reproducbility of random numbers
n <- 100

# Create a data frame with two variables: age and smoker
df <- data.frame(
  age <- rnorm(n, mean = 35, sd = 10),
  smoker <- sample(c("Yes", "No"), n, replace = TRUE, prob = c(0.25, 0.75))
)
df <- df |>
  mutate(smoker = ifelse(age < 40, "Yes", smoker))
```
...
Analyse the relationship between dipl_aucun and proprio
...
<!-- - Logistic regression is a statistical model used for for modelling the relationship between a binary dependent variable and one or more independent variables -->
...
<!-- <div class="supplementary"> -->
<!-- The coefficients are odds ratios: for a one unit increase in the independent variable, the odds of the outcome occurring are $e^\beta$ times larger, where $\beta$ is the coefficient for the independent variable. If $β = 1.3$, then $e^\beta = 3.675$. This means that for each additional unit of $x$, the odds of $y=1$ are 3.67 times higher.   -->
...
<!-- <div class="supplementary"> -->
<!-- Exponentiating the coefficient transforms it from log-odds back into plain odds. This value tells you how the odds of being in the specified category (compared to the reference category) change with a one-unit increase in the independent variable. A value above 1 indicates an increase in odds, a value below 1 indicates a decrease, and a value of 1 means no change.   -->
...
<!-- Other example: a dataset of students, schools, and districts. Students are nested within schools, and schools are nested within districts. You're interested in predicting a student's test score based on their age and the school's average class size.   -->
<!-- Assuming a dataframe 'data' with columns 'score', 'age', 'class_size', 'student', 'school', and 'district' -->
<!-- ``` -->
<!-- model <- lmer(score ~ age + class_size + (1|school/district), data = data) -->
<!-- ``` -->
<!-- - score ~ age + class_size is specifying the fixed effects part of the model. We're modeling the test score as a function of the student's age and the school's average class size. -->
<!-- - (1|school/district) is specifying random intercepts for schools and districts. The / notation is a shorthand for nested grouping factors, so this is equivalent to (1|school) + (1|district:school). Each school can have a different baseline test score, and each district can have a different baseline test score that adds to the school effect. -->

<!-- This model assumes that the average test score might vary from district to district and from school to school within each district. The differences between districts and schools are captured by the random intercepts. The model also assumes that the effect of a student's age and the school's average class size on test scores is fixed, i.e., the same in all districts and schools. -->

<!-- --- -->

<!-- ### Principal Components Analysis (PCA) -->

<!-- Principal Components Analysis (PCA): technique used for reducing the dimensionality of a dataset while retaining as much of the original variation as possible.  -->
<!-- This is accomplished by identifying linear combinations of the original variables, called principal components, which account for the most variance in the data. -->

<!-- ```{r, eval=F} -->
<!-- library(FactoMineR)  # perform the analysis -->
<!-- library(factoextra)  # extract information for the analysis and provides visualizations (fviz_*) -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- set.seed(123) -->
<!-- pca_data <- data.frame(matrix(rnorm(50*15), nrow=50))  # 50 indiv, 15 variables -->
<!-- colnames(pca_data) <- paste0("var", 1:15) -->
<!-- rownames(pca_data) <- paste0("indiv", 1:50) -->
<!-- ``` -->

<!-- --- -->

<!-- ```{r, fig.height=4} -->
<!-- pca_res <- PCA(pca_data, scale.unit=TRUE, graph=F)   -->
<!-- # scale.unit=TRUE to have unit variance before the analysis -->
<!-- fviz_screeplot(pca_res)   -->
<!-- # The elbow method to decide how many axes to keep, based on a "scree plot" -->
<!-- ``` -->

<!-- --- -->

<!-- To give an interpretation to the axes, it is common to display the variables on the axes -->
...
<!-- To get the coordinates of individuals on the axes -->
...
<!-- Multiple Correspondence Analysis (MCA) is a technique used for analyzing the association between categorical variables. It is a type of PCA that is specifically designed for categorical data. -->
<!-- It allows to reduce the number of categorical variables and to find relationships between these variables. MCA is often used in survey or polling data analysis where the responses are categorical. -->

<!-- Creation of a dataset -->

<!-- ```{r} -->
<!-- mca_data <- data.frame(matrix(sample(0:5, 100*15, replace = TRUE), ncol = 15))   -->
<!-- # 100 indiv, 15 variables, likert scale (0 to 5) -->
<!-- colnames(mca_data) <- paste0("var", 1:15) -->
...
<!-- ```{r, fig.height=4} -->
<!-- fviz_mca_var(mca_res, choice="var") -->
<!-- ``` -->

<!-- --- -->

<!-- ```{r} -->
<!-- head(mca_res$ind$coord) -->
<!-- ``` -->
...
<!-- Load the dataset questionnaire.rdata and perform a PCA and a MCA.    -->
<!-- <http://www.duboishome.info/dimitri/cours/edeg/R/files/questionnaire.rdata> -->

<!-- The dataset is composed of responses from 248 individuals to 39 questions. Responses ranges from 0 to 5. -->
...
<!-- ```{r, fig.height=4} -->
<!-- ggplot() +  -->
<!--   geom_point(aes(x=data[,1], y=data[,2], col=colors()[k$cluster])) +  -->
<!--   geom_point(aes(x=k$centers[,1], y=k$centers[, 2], col=colors()[1:3]), shape=3, size=5) +  -->
<!--   labs(x="", y="", color="Clusters") + guides(color="none") -->
<!-- ``` -->
...
<!-- Create 3 clusters of participants based on their responses in the questionnaire. Then display the individuals wrt their coordinates in the two first axes of the PCA analysis. -->


# Sources {#sources}

-   <https://www.r-tutor.com/r-introduction>
-   a complete book on data analysis with R [here](https://larmarange.github.io/analyse-R/analyse-R.pdf)
-   the class of Julien Barnier [here](https://juba.github.io/tidyverse/index.html)
-   the website <https://www.tidyverse.org/>
-   the website [R for data science](https://r4ds.had.co.nz/index.html)
-   the website dedicated to ggplot2 <http://ggplot2.tidyverse.org/reference/>
